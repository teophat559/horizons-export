...
const initialContests = [
  { id: 1, name: 'Giọng Ca Vàng 2025', description: 'Cuộc thi tìm kiếm tài năng âm nhạc đỉnh cao, nơi các giọng ca tranh tài để giành ngôi vị quán quân.', bannerUrl: 'https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?q=80&w=2070&auto=format&fit=crop' },
  { id: 2, name: 'Hoa Hậu Hoàn Vũ 2025', description: 'Hành trình tôn vinh vẻ đẹp, trí tuệ và lòng nhân ái của phụ nữ Việt Nam trên đấu trường quốc tế.', bannerUrl: 'https://images.unsplash.com/photo-1599485659203-f7f59556548a?q=80&w=1974&auto=format&fit=crop' },
  { id: 3, name: 'Startup Kỳ Lân 2025', description: 'Sân chơi cho các nhà khởi nghiệp trẻ thể hiện ý tưởng đột phá và kêu gọi vốn từ các nhà đầu tư hàng đầu.', bannerUrl: 'https://images.unsplash.com/photo-1556761175-5973dc0f32e7?q=80&w=1932&auto=format&fit=crop' },
];
...
const ContestsPage = () => {
  const [contests, setContests] = useState(() => {
    try {
      ...
      const parsed = saved ? JSON.parse(saved) : initialContests;
      return Array.isArray(parsed) && parsed.length > 0 ? parsed : initialContests;
    } catch (error) {
  ...
  const [formData, setFormData] = useState({ name: '', description: '', bannerUrl: '' });
  ...
  const openEditDialog = (contest) => {
    ...
    setFormData({ name: contest.name, description: contest.description, bannerUrl: contest.bannerUrl });
    setIsDialogOpen(true);
  };
                  ...
                  <TableBody>
                    {filteredContests.map(c => (
                      <TableRow key={c.id} className="border-gray-800/80 text-white interactive-glow hover:bg-transparent cursor-pointer">
                        <TableCell>
                          <img src={c.bannerUrl || 'https://via.placeholder.com/40'} alt={c.name} className="h-10 w-20 rounded-lg object-cover" />
                        </TableCell>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px] bg-[#0f0c29] border-purple-800 text-slate-50">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="bannerUrl" className="text-slate-400">URL Banner</Label>
              <Input id="bannerUrl" value={formData.bannerUrl} onChange={handleFormChange} className="bg-slate-800/50 border-slate-700" />
            </div>
          </div>
          <DialogFooter>
